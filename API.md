...
| `{"type":"account_update", "billing"?}` | same `account` frame, updated. `billing` is free-form client prefs **EXCEPT `billing.sub`, which is SERVER-OWNED** (the payment webhook writes it; a client-supplied `sub` is silently stripped) — so a client can't spoof subscription state. |
| `{"type":"checkout_create", "plan"?}` | `{"type":"checkout", "url"}` — a Paddle **hosted-checkout** URL for this account (our account id in `custom_data`; Paddle never sees a profile). v1 uses one env-configured plan; `plan` optional. Only when the account plane advertises the `billing_checkout` feature (`hello`). Errors: `billing_disabled`, `billing_error`. |
| `{"type":"billing_portal"}` | `{"type":"portal", "url"}` — a Paddle customer-portal deep link (manage/cancel/update card, receipts). Needs a known `billing.sub.customer_id` (set by the webhook after a purchase). Errors: `no_customer`, `billing_disabled`, `billing_error`. |
...
| `{"type":"paying_attest"}` | `{"type":"paying_attestation", "attestation"}` — a bearer attestation purpose-tagged `payingverify` for the **`paying`** badge (account-level credibility). Requires a **paying** account (set by billing or `account_server/set_paying.py`). **Unbudgeted and repeatable** — unlike `irl`, it is meant to **flow to ALL the account's profiles** (each redemption unlinkable to the account and to the others). No DID/account reference (the firewall). The granted `paying` badge **expires after 72h** (unlike permanent `email`/`irl`): the client must periodically re-run `paying_attest` → `profile_attest` per profile to refresh it, which only succeeds while the account is currently paying — so a lapsed subscriber de-badges within 72h. Error: `account is not a paying customer` (`code:"not_paying"`). |
...
| `{"type":"watch", "rooms":[..]}` | `{"type":"watching", "rooms":[..]}` — **multi-room live subscribe** (feature **`multi_room_watch`**): add LIVE frames for a set of rooms you're a **member** of to this ONE connection without `join`ing each — a corners channel list keeps board + lobby + N channels ticking unread over one socket. **REPLACE** semantics: the watch set becomes exactly the accepted rooms; membership-checked, bounded (`CHAT_WATCH_MAX`, default 64), and your active `join`ed room is **excluded** (it's already live). **Live-only** — each watched room goes live from its current tail; the client `sync`s each room's own backlog. Frames carry **`room`**, so route by it; private rooms deliver **sealed** frames you decrypt with that room's key. The reply `rooms` is the exact current watch set (unknown / non-member / invalid names are silently dropped); `watch` with `[]` clears it. Cleared automatically on disconnect. |
...
| `{"type":"corner_info", "corner"}` | `{"type":"corner_info", "corner", "board", "lobby", "private", "join_policy", "sponsor", "sponsor_did", "members", "rooms":[{"name","private"},..], "joined"}` — read a corner (public read). `members` = roster size, `sponsor` = current @handle, **`rooms`** = the corner's CHANNELS (excludes board + lobby, for the rooms rail), **`joined`** = is the caller on the roster. |
| `{"type":"corner_list"}` | `{"type":"corner_list", "corners":[{"corner","board","lobby","private","members","joined","rooms":[..]}, ..]}` — enumerate all **PUBLIC** corners plus any private ones the caller belongs to; **`joined`** lets the client split a "Yours" section from discovery. (At launch, one corner → `[commons]`.) |
